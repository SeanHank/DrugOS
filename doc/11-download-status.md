...
| 10 | In-vitro toxicity assay parameters (BSEP inh., ETC, oxidative stress) | DILIsym literature consortia | **NOT-DOWNLOADED** | cited papers (numerical tables) |
...
- **VENDORED (checksum-pinned): 3** files (`published_pk.json`, measured hERG
  submation `herg_measured_nm.json`, `corpora/herg_central.tsv.gz`).
- **VIA-PKG (always available with the install): 2** (`admet-ai`, `rdkit`).
- **On-disk effective datasets: 5 units** serving 5 benchmarks + custom SMILES
  (published PK bands, per-compound measured hERG, 306 k-row hERG corpus).
...
- **N/A (implemented priors, code-is-the-truth): 4** (rows 5, 18 fronts, 19).
