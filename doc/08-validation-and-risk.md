...
| Predictive-regime reliability (D25/D26) | The DISCLAIMER §2 clause is executable: six predictive regimes (weakest evidence axis dominates) disclose `trust.reliability` (regime/label/basis/band_cv) on every run; a *weaker* regime can never report a narrower parameter-ensemble band; measured-true-parameter overrides upgrade a run and are actually used (not a synthesized stand-in); observed-vs-predicted disagreement is reported as fold-error / within-2x rows in `trust.empirical_agreement` | implemented (L1, `case_predictive_regime`) |
...
- **G4 rule:** every new model feature must add a validation case before merge; `python validation/run_validation.py` (38/38 cases green, see `validation/report.md`) regenerates the report and fails the gate on any red case.
...
- **E2E full-chain case** (`case_full_chain_admet_to_report`): runs the real chain — structure → ADMET-AI → `spec_from_admet` → `run_pipeline` → `to_contract` → rendered/written report — on two unrelated molecules; asserts contract determinism, the eight-section contract shape, `Cmax > 0`, `Fa ∈ (0,1]`, the per-molecule **trust record**, and that every report artifact is written. A missing ADMET-AI runtime is a hard FAIL (no silent fallback).
- **Trust record** (`fidelity_provenance`, `doc/12 §7`): every contract carries a `trust` section disclosing the `fidelity` lane (`full` default with every realism term engaged and its auto-anchor basis in `estimates`, or the explicit `baseline` opt-out), the terms that could not engage (`mechanism_terms_degraded`; empty in full — a full run that would drop a term raises `RealismError`), the wired production anchors (R-4/R-6/R-7/R-8 as applicable), class-prior sites, contributed ADMET-AI heads, a **predictive-regime reliability block** (`trust.reliability`, DISCLAIMER §2 — regime, label, reliability, basis, band_cv, disclaimer) and, when supplied, an **observed-data agreement block** (`trust.empirical_agreement` — per-endpoint fold-error / within-2x rows plus the written policy that disagreement is the expected state of the mechanistic model). This is the machine-readable form of the no-silent-fallback rule.
