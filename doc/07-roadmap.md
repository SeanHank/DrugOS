...
- **Exit criteria:** ✅ reproducible benchmark suite passes (28/28 cases green, GMFE table in `validation/report.md`; includes R-1 R-PK, R-2 corpus calibration, R-3 ORd cardiac cross-check, R-4 BBB/CNS partition, R-5 SBML MAPK lane, R-6 CKD-EPI GFR, R-7 bile-acid cholestasis PBK).
...
- **Required-R hard dependency** — literature PK estimator executed by R ≥ 4 on
  every run, agreement gated ≤ 2 % (validation R-1, 28/28 cases).
