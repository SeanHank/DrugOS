...
- **Exit criteria:** ✅ reproducible benchmark suite passes (36/36 cases green, GMFE table in `validation/report.md`; includes R-1 R-PK, R-2 corpus calibration, R-3 ORd cardiac cross-check, R-4 BBB/CNS partition, R-5 SBML MAPK lane, R-6 CKD-EPI GFR, R-7 bile-acid cholestasis PBK, R-8 calibrated hERG head vs corpus, `case_cheng_prusoff_conversion` IC50→Ki, `case_cyp_kinetics` per-CYP MM/Hill, `case_tmdd_drug_disposition` native TMDD coupling, `case_transdermal_multi_layer` multi-layer skin permeation, `case_cardiac_sympathetic_suppression` sympathetic-suppression branch, `case_dili_immune_activation` immune-mediated DILI, and `case_acat_multisegment_si` multi-segment SI dissolution).
...
- **Required-R hard dependency** — literature PK estimator executed by R ≥ 4 on
  every run, agreement gated ≤ 2 % (validation R-1, 36/36 cases).
...
- **Multi-layer transdermal skin permeation** — finite-dose membrane
  ...
  replaces the generic transdermal depot when `SkinLayers` is opt-ed in
  (off by default; PBPKModel, `case_transdermal_multi_layer` — Fick
  steady-state flux, interface partitions, barrier/diffusivity, mass closure).
- **Sympathetic-suppression cardiac branch** — the former "explicitly
  deferred" suppression axis (ERK encodes only stimulation) is now a
  saturable Emax beta-adrenergic blockade on heart rate and stroke volume
  (`CardiacParams.sympathetic_tone`, off by default; `RunSpec`,
...
- **Immune-mediated DILI axis** — the former doc/05 4.2 deferred stub is now
  ...
  off by default; `RunSpec`, `dili_immune_ic50_nm` + `dili_immune_weight`)
...
- **ACAT-lite multi-segment SI** — the former "no multi-segment small-
  ...
  ``AbsorptionParams.si_segments`` (off by default): N sequential equal-
  ...
  solubility cap, segment-count sensitivity, off-by-default, degenerate
  rejection).
